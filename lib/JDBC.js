...
class JDBC {
    ...
    createStatement(connectIfClosed) {
        return this.getConnection(connectIfClosed).then((connection) => connection.createStatement());
    }
    closeConnection() {
        if (this.connection) {
    ...
    validateConfig() {
        if ((0, lodash_1.isEmpty)(this.config.className)) {
    ...
    newConnection() {
        return Java_1.Java.getInstance()
            .java.callStaticMethodAsync("java.sql.DriverManager", "getConnection", this.config.url, this.config.username || null, this.config.password || null)
